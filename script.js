...
const seletNum = function () {
  if (clicked) {
    ...
    window.addEventListener("keydown", function (e) {
      if (clicked) {
        checkForIncorrect(gridTextcontent, checkSquare(false), e.key);
      }
...
function checkSquare(answerArray = true) {
  ...
  gridTextcontent.forEach((row, i) => {
    row.forEach((col, j) => {
      if ((col.style.backgroundColor = "wrong")) {
        coards.push(i, j);
      }
    });
  });
...
function checkForIncorrect(board, coards, chekingAnswer) {
  ...
  const checkRow = board.map((row, i) => {
    if (i == rowNumber) {
      return row.map((col, j) => {
        if (row[i][j] == chekingAnswer) {
          console.log(true);
          return "foo";
        }
        return col;
